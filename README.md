...
Класс Page
Page - управляет отображением основных блоков страницы: каталога, корзины, форм и модальных окон.
Поля:catalogContainer: HTMLElement - контейнер с карточками товаров; cartContainer: HTMLElement - контейнер корзины; modal: Modal - экземпляр модального окна;events: EventEmitter - брокер событий.
Методы:renderCatalog(items: HTMLElement[]): void - отрисовывает карточки каталога; renderCart(items: HTMLElement[]): void - отрисовывает карточки корзины; updateTotalPrice(price: number): void - обновляет общую сумму заказа; openModal(content: HTMLElement): void - открывает модальное окно.
